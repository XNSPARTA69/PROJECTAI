...
# I create the uploads folder if it doesn't exist
...
# I try to start Ollama as a background process
def start_ollama():
    try:
        # For Windows
        if os.name == "nt":
...
# I check if llama3.1:latest is available
def is_llama31_available():
    try:
        response = requests.get(f"{OLLAMA_HOST}/api/tags", timeout=2)
        if response.ok:
            tags = response.json().get("models", [])
            # I look specifically for llama3.1:latest
            return any("llama3.1:latest" in m.get("name", "") for m in tags)
        return False
    except Exception:
        return False

# I start Ollama and ensure the llama3.1:latest model is available
def ensure_ollama_and_llama31():
    if not is_ollama_running():
        print("Ollama is not running, starting...")
        if not start_ollama():
            raise RuntimeError("Failed to start Ollama automatically.")
...
# I load the YOLO model
def load_yolo_model():
    return YOLO('yolov8n.pt')

# I load the MONAI model
...
def process_image():
    try:
        # --- I verify Ollama and llama3.1:latest ---
        ...
        # --- MONAI transformations and segmentation CORRECTED ---
        # If the image is 2D, I add the channel first; if it already has a channel, I don't
        if grayscale_image.ndim == 2:
            monai_image = np.expand_dims(grayscale_image, axis=0)  # (1, H, W)
        else:
            monai_image = grayscale_image  # It already has a channel

        # I apply transformations (without EnsureChannelFirst)
        ...
        if monai_image.ndim != 4:
            return jsonify({'error': f'Invalid image dimensions: {monai_image.shape}'}), 400
        ...
        # I process data for LLaMA 3.1:latest
        detection_summary = f"{len(detections)} objects detected (prosthetics, limbs, or others)."
        segmentation_summary = f"Segmented area: {int(np.sum(segmentation > 0.5))} pixels."

        # Query to Ollama/Llama 3.1:latest
        try:
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "describe any potential issues with the prosthetic fit or anomalies in the image. "
                            "Be concise, clear, and helpful for a non-technical user. "
                            "Respond with soft words and emotional support. "
                            f"Detections: {detection_summary}\n"
                            f"Segmentation: {segmentation_summary}\n"
                            "Example: 'The prosthetic appears misaligned at the top, which could cause discomfort.'"
...
# --- NEW: Route for text-AI interaction (chat with LLaMA 3.1) ---
...
def chat():
    ...
    # Only call LLaMA 3.1 if there is text input
    try:
...
if __name__ == '__main__':
    ...
    ###MISSING TRAINING MONAI FOR IMAGES